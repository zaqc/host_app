...
static void dwav_usb_mt_report(struct dwav_usb_mt *dwav_usb_mt)
{
	...
	for (id = 0; id < DWAV_TOUCH_MAX_ID; id++)	{
		//printk(KERN_DEBUG "status=%i x=%i y=%i", dwav_usb_mt->finger[id].status
		//    ,dwav_usb_mt->finger[id].x, dwav_usb_mt->finger[id].y);
		...
		unsigned char *pkt, int len)
{
	...
	for(i = 0; i < 1; i++) {
	    //if(i == 0) {
		x = (*(unsigned short *)&pkt[3 + i * 4]);
		y = (*(unsigned short *)&pkt[5 + i * 4]);
	    //} else {
		//x = (*(unsigned short *)&pkt[5 + i * 4]);
		//y = (*(unsigned short *)&pkt[7 + i * 4]);
	    //}

	    if(x > 0 && x < 800 && y > 0 && y < 480 && pkt[2] == 1) {
		// printk(KERN_DEBUG "STEP=[%i] DM=%i ID=%i ST=%i, x=%i y=%i ", i, (int)pkt[0], (int)pkt[1], (int)pkt[2], (int)x, (int)y);
		input_report_abs(dwav_usb_mt->input,  ABS_X, x);
		input_report_abs(dwav_usb_mt->input,  ABS_Y, y);
		input_report_key(dwav_usb_mt->input, BTN_TOUCH, pkt[1] == 7);
		input_sync(dwav_usb_mt->input);
	    }
	}
	    

	//printk(KERN_DEBUG "hdr=0x%02X x1=%i y1=%i",
	//    dwav_raw->header, dwav_raw->x1, dwav_rav->y1);
...
static void dwav_usb_mt_irq(struct urb *urb)
{
	...
	//printk(KERN_DEBUG "irq... %i", urb->status);
